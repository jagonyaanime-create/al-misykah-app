...
<div class="max-w-5xl mx-auto py-8 px-4">
    ...
    <div class="mb-8 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-slate-800">Edit Data Santri</h1>
            <p class="text-slate-500 mt-1">Memperbarui informasi untuk santri: <span class="font-bold text-emerald-600">{{ $santri->nama }}</span></p>
        </div>
        <div class="hidden md:block">
            <span class="bg-slate-100 text-slate-600 px-4 py-2 rounded-xl text-xs font-bold uppercase">ID: {{ $santri->id }}</span>
        </div>
    </div>

    <!-- Gunakan Route Update dengan Method PUT -->
    <form action="{{ route('santri.update', $santri->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-6">
                <div class="bg-white p-6 rounded-[32px] shadow-sm border border-slate-200">
                    ...
                    <!-- Input Foto -->
                    <div x-data="{ photoPreview: '{{ $santri->foto ? asset('images/santri/'.$santri->foto) : null }}' }" class="mb-6">
                        ...
                        <div class="relative group">
                            <div class="w-full h-48 rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden transition-all group-hover:border-emerald-400">
                                <!-- Tampilkan Foto Lama jika ada dan Belum ada Preview Baru -->
                                <template x-if="!photoPreview">
                                    <div class="text-center text-slate-400">
                                        <i class="fa-solid fa-camera text-3xl mb-2"></i>
                                        <p class="text-xs">Klik untuk ubah foto</p>
                                    </div>
                                </template>
                                <!-- Tampilkan Preview Foto Baru/Lama -->
                                <template x-if="photoPreview">
                        ...
                        <p class="text-[10px] text-slate-400 mt-2 italic text-center">Abaikan jika tidak ingin mengubah foto</p>
                    </div>

                    <!-- Input Status -->
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Status Keaktifan</label>
                        <select name="status" class="w-full rounded-xl border-slate-200 focus:ring-emerald-500 text-sm font-medium">
                            ...
                            <option value="berhenti" {{ $santri->status == 'berhenti' ? 'selected' : '' }}>Berhenti</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="nama" value="{{ old('nama', $santri->nama) }}" required class="w-full rounded-xl border-slate-200 focus:ring-emerald-500 focus:border-emerald-500" placeholder="Masukkan nama sesuai ijazah...">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">NIS (Nomor Induk)</label>
                            <input type="text" name="nis" value="{{ old('nis', $santri->nis) }}" required class="w-full rounded-xl border-slate-200 focus:ring-emerald-500 focus:border-emerald-500" placeholder="Contoh: 12345678">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $santri->tempat_lahir) }}" required class="w-full rounded-xl border-slate-200" placeholder="Contoh: Grobogan">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Tanggal Lahir</label>
                            <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $santri->tanggal_lahir) }}" required class="w-full rounded-xl border-slate-200 focus:ring-emerald-500">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-[32px] shadow-sm border border-slate-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="kelas" value="{{ old('kelas', $santri->kelas) }}" required class="w-full rounded-xl border-slate-200" placeholder="Contoh: 3 Tsanawiyah">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Kelas Muhadhoroh</label>
                            <input type="text" name="kelas_muhadhoroh" value="{{ old('kelas_muhadhoroh', $santri->kelas_muhadhoroh) }}" class="w-full rounded-xl border-slate-200" placeholder="Contoh: 3 Muhadhoroh">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Kamar / Asrama</label>
                            <input type="text" name="kamar" value="{{ old('kamar', $santri->kamar) }}" required class="w-full rounded-xl border-slate-200" placeholder="Contoh: Abu Bakar Ash-Shidiq">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-emerald-700 mb-2 p-3 bg-emerald-50 rounded-xl">
                                <i class="fa-solid fa-circle-info mr-2"></i> Hubungkan ke Akun Login Wali
                            </label>
                        ...
                        <div>
                            ...
                            <input type="text" name="wali" value="{{ old('wali', $santri->wali) }}" required class="w-full rounded-xl border-slate-200" placeholder="Nama Bapak/Wali">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Nomor WhatsApp Wali</label>
                            <input type="text" name="no_hp" value="{{ old('no_hp', $santri->no_hp) }}" required class="w-full rounded-xl border-slate-200" placeholder="08xxxxxxxxxx">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Alamat Lengkap Rumah</label>
                            <textarea name="alamat" rows="3" required class="w-full rounded-xl border-slate-200" placeholder="Tuliskan alamat lengkap...">{{ old('alamat', $santri->alamat) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <!-- Error Messages -->
                @if ($errors->any())
                <div class="bg-red-50 border-l-4 border-red-500 p-4 mb-6 rounded-r-xl text-red-700">
                    <p class="font-bold mb-1 text-sm">Ada masalah saat memperbarui data:</p>
                ...
                <div class="flex items-center justify-between pt-6 border-t mt-8">
                    ...
                    <div class="flex space-x-3">
                        <button type="submit" class="bg-blue-600 text-white px-12 py-3 rounded-2xl font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition active:scale-95">
                            Update Data Santri
                        </button>
                    </div>
                </div>
